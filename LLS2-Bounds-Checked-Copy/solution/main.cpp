...
enum class CopyStatus { Ok, Truncated, InvalidInput };
...
CopyResult copyUserLabel(std::string_view input,
    ...
    return {input.size() > maxVisibleChars ? CopyStatus::Truncated
                                           : CopyStatus::Ok,
            amountToCopy};
}
...
void printBytes(const std::array<char, 16>& destination) {
    ...
    for (std::size_t index = 0; index < destination.size(); ++index) {
        ...
        std::cout << static_cast<int>(
            static_cast<unsigned char>(destination[index]));
    }
    std::cout << "]";
}
