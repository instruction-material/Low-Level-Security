...
int main() {
    ...
    std::cout << "stack pair bytes   = "
              << byteDistance(&stackValue, &secondStackValue) << "\n";
    std::cout << "stack/global bytes = "
              << byteDistance(&stackValue, &gGlobalCounter) << "\n";
    std::cout << "heap/vector bytes  = "
              << byteDistance(heapValue.get(), values.data()) << "\n";

    std::cout << "\nExtend this lab:\n";
    std::cout << "1. Add one more helper-function local and compare it to "
                 "main's stack addresses.\n";
    std::cout << "2. Record which regions are tied to scope versus program "
                 "lifetime.\n";
    std::cout << "3. Explain which addresses changed between runs and why that "
                 "does not change storage class.\n";
}
