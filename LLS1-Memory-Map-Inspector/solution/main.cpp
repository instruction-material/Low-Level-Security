...
void printSample(const RegionSample& sample) {
    std::cout << std::left << std::setw(16) << sample.label
              ...
              << " address=" << sample.address << " note=" << sample.note
              << "\n";
}
...
int main() {
    ...
    const std::vector<RegionSample> samples{
        ...
        {"heapValue", heapValue.get(), "heap", "owned",
         "released by unique_ptr"},
        {"heapBuffer", heapBuffer->data(), "heap", "owned",
         "separate from local variables"},
        ...
        {"readOnly", gReadOnlyLabel, "rodata", "program",
         "read-only string literal area"},
        {"string data", text.data(), "heap-ish", "object",
         "managed by std::string"},
    ...
    std::cout << "\nObservations:\n";
    std::cout << "- Stack locals cluster together because they share a "
                 "call-frame lifetime.\n";
    std::cout << "- Heap allocations are separate from the stack and must be "
                 "owned explicitly.\n";
    std::cout << "- Global, static, and read-only data live for the whole "
                 "process lifetime.\n";
    std::cout << "- Address changes between runs do not change the storage "
                 "class or lifetime rules.\n";
}
