...
std::string exploitabilityNote(const BugCase& bug, CrashClass classification) {
    ...
    stream << "reachable=" << (bug.reachableFromUntrustedInput ? "yes" : "no")
           << ", attacker-control="
           << (bug.attackerControlsShape ? "high" : "low") << ", mitigations="
           << (bug.mitigationsPresent ? "present" : "limited")
           << ", impact=" << toText(classification);

    if (classification == CrashClass::MemoryCorruption) {
        stream << ". Treat as security-relevant because attacker-shaped input "
                  "can corrupt adjacent control or state data.";
    } else if (classification == CrashClass::MemoryDisclosure) {
        stream << ". Treat as disclosure risk because stale or unintended "
                  "bytes cross a trust boundary.";
    } else if (classification == CrashClass::DenialOfService) {
        stream << ". Prioritize availability impact and confirm whether the "
                  "crash can be triggered repeatedly.";
    } else {
        stream << ". Keep the note short and explain why current evidence does "
                  "not show disclosure or corruption.";
    }
