...
std::vector<std::uint8_t> mutate(const std::vector<std::uint8_t>& seed,
    ...
    std::uniform_int_distribution<std::size_t> indexDist(0,
                                                         candidate.size() - 1);
    ...
    if (action == 0) {
        ...
        if (candidate.size() > 2 && (byteDist(engine) % 2 == 0)) {
            ...
            candidate.push_back(
                static_cast<std::uint8_t>(printableDist(engine)));
        }
    }
...
int main() {
    ...
    for (int round = 0; round < 100; ++round) {
        ...
        if (status != ParseStatus::Ok && interestingCases.size() < 8) {
            interestingCases.push_back(packetToHex(candidate) + " -> " +
                                       toText(status));
        }
    }

    std::cout << "ok=" << stats.ok << " too-short=" << stats.tooShort
